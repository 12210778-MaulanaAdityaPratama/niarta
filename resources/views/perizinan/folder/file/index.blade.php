...
@section('konten')
    ...
    <div class="mb-3">
    <a href="{{ route('file.tambah', ['folder_id' => $folder->id]) }}" class="btn btn-primary">Tambah File</a>
    </div>
    <form action="{{ route('file.index', ['folder_id' => $folder->id]) }}" method="GET">
        <div class="input-group mb-3">
            ...
            <a href="{{ route('file.index', ['folder_id' => $folder->id]) }}" class="btn btn-outline-secondary">Clear</a>
        </div>
    </form>
            ...
            <tbody>
                @foreach ($files as $file)
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('file.download', ['file_id' => $file->id]) }}" class="btn btn-primary">Unduh</a>
                            <!-- Tambahkan tombol atau tautan lainnya sesuai kebutuhan -->
                           <!-- Tambahkan tombol hapus -->
                           <form action="{{ route('file.destroy', ['file_id' => $file->id]) }}" method="POST" style="display:inline;">
                            @csrf
                            @method('DELETE')
    ...
    <a href="{{ route('perizinan.view', ['id' => $folder->perizinan->id]) }}" class="btn btn-primary">Kembali</a>
@endsection
